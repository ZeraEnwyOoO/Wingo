...
static const char *log_level_colors[] = {
    WINGO_COLOR_DIM,      /* TRACE */
    WINGO_COLOR_CYAN,     /* DEBUG */
    WINGO_COLOR_GREEN,    /* INFO */
    WINGO_COLOR_BLUE,     /* NOTICE */
    WINGO_COLOR_YELLOW,   /* WARN */
    WINGO_COLOR_RED,      /* ERROR */
    WINGO_COLOR_RED WINGO_COLOR_BOLD,  /* FATAL */
    WINGO_COLOR_RESET     /* NONE */
};
 ...
 *
 * We use a recursive mutex because:
 *   - log_write can call log_flush
 *   - log_flush can call log_rotate
 *   - log_rotate can call log_write (for error messages)
 */
...
static void log_thread_id(char *buf, wingo_size size)
{
    ...
    /*
     * pthread_self() returns pthread_t which is opaque.
     * Cast to unsigned long for display purposes.
     */
 ...
 *
 * Returns true if file size exceeds max_file_size.
 */
...
static wingo_error_t log_rotate_files(void)
{
    ...
    for (i = log_state.max_files - 1; i >= 1; i--) {
        ...
        if (rename(old_path, new_path) != 0 && errno != ENOENT) {
            /* Log to stderr since our file is closed */
...
/*
 * Write a formatted log line to a FILE.
 */
...
wingo_error_t wingo_log_init(const wingo_log_config_t *config)
{
    ...
    /* Open log file if target includes FILE */
    ...
    /* Initialize syslog if target includes SYSLOG */
                      ...
                      va_list args)
{
     ...
     * We check before locking to avoid contention.
     */
     ...
     *
     * Format:
     *   [timestamp] [TID] LEVEL [file:line] func: message
     *
     * We build it piece by piece to handle optional fields.
     */
    pos = 0;
                   ...
                   wingo_size len)
{
    ...
    /*
     * Print in chunks of 16 bytes per line.
     * Format: 0000: 00 11 22 33 44 55 66 77  88 99 aa bb cc dd ee ff  |.......|
     */
